...
Changes that can affect the desktop bundle trigger a build-only validation matrix. The same build-only matrix can be started with `workflow_dispatch`.

Current validation outputs:

- Linux x64 on Ubuntu 22.04: `.deb` and `.AppImage`
...
Validation jobs use read-only repository permissions and upload the generated bundles only as workflow artifacts. They do not create a GitHub Release.

Ubuntu 22.04 is intentionally used as the Linux packaging baseline because AppImage compatibility is constrained by the build system's glibc baseline. Normal compile/test CI may use newer Linux images independently.

## Tagged draft releases

A push of a version tag matching `v*` runs the same platform/architecture matrix with release permission scoped only to the tag-only draft-release jobs. The workflow creates or updates a **draft prerelease** and uploads the produced bundles.
...
A successful bundle job proves that the package can be produced on the declared runner. It does not prove trust-store acceptance, semantic text-insertion compatibility, or support on every target machine/application.
